...
template <typename T, typename V> struct Pair
{
    ...
    constexpr Pair(U &&first, W &&second);
    template <typename U, typename W>
        requires(SameAs<U, T> && SameAs<W, V>)
    constexpr Pair(Pair<U, W> &&other);
    template <typename U, typename W>
        requires(SameAs<U, T> && SameAs<W, V>)
    constexpr Pair<T, V> &operator=(Pair<U, W> &&other);
};
...
template <typename U, typename W>
    requires(SameAs<U, T> && SameAs<W, V>)
constexpr Pair<T, V>::Pair(U &&first, W &&second)
...
template <typename U, typename W>
    requires(SameAs<U, T> && SameAs<W, V>)
constexpr Pair<T, V>::Pair(Pair<U, W> &&other)
...
template <typename U, typename W>
    requires(SameAs<U, T> && SameAs<W, V>)
constexpr Pair<T, V> &Pair<T, V>::operator=(Pair<U, W> &&other)
{
    if (this != &other)
    {
        first = WHEELS_MOV(other.first);
...
[[nodiscard]] constexpr bool operator==(
    Pair<T, V> const &lhs, Pair<T, V> const &rhs)
{
